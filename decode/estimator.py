...
class ResEstimator:
    def __init__(self, model_path, net, inp_dim=224 , device = 'cpu'):
        self.inp_dim = inp_dim
        self.net = net.to(device)
        self.device = device
        #self.net.load_state_dict(torch.load(model_path, map_location=lambda storage, loc: storage)[['net_state_dict']])
    ...
    def inference(self, in_npimg):
        ...
        keypoints = dsntnn.dsnt(dsntnn.flat_softmax(heat_map[5]))
