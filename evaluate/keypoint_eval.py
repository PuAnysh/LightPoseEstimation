...
def keypoint_eval(predictions, annotations, return_dict):
    ...
    for image_id in annotations['image_ids']:
        ...
        if image_id in prediction_id_set:
            oks = compute_oks(anno=annotations['annos'][image_id],predict=predictions['annos'][image_id]['keypoint_annos'],delta=annotations['delta'])
    ...
    for threshold in np.linspace(0.5, 0.95, 10):
        ...
        print('AP@{:.2f} is {}'.format(threshold,np.sum(oks_all > threshold) / np.float32(oks_num)))
